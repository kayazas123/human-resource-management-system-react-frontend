...
export default function UpdateEmployeeProfile(props) {
    ...
    const [middleName, setMiddleName] = useState(employeeDetails.middleName);
    ...
    const [country, setCountry] = useState(employeeDetails.countryId)
    const [countries, setCountries] = useState([]);
    const [dateOfBirth, setDateOfBirth] = useState(new Date(employeeDetails.dateOfBirth))
    ...
    const submitDetailsForUpdation = () => {
        if (areDetailsFilled) {
            ...
            var data = JSON.stringify({ "employeeId": props.userId, "firstName": firstName, "middleName": middleName, "lastName": lastName, "description": description, "status": status, "gender": gender, "dateOfBirth": dateOfBirth, "countryId": country });

            var config = {
                method: 'put',
                url: 'http://localhost:9090/v1/employee-details',
                headers: {
